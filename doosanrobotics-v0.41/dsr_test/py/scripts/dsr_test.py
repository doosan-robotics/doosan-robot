...
if __name__ == "__main__":
    rospy.init_node('dsr_test_py')
    rospy.on_shutdown(shutdown)

    print("xxxx")
    robot_id    = 'dsr01'
    robot_model = 'm1013'
    srv_name_prefix   = '/' + robot_id + robot_model  
    topic_name_prefix = srv_name_prefix
    print("srv_name_prefix = %s" % srv_name_prefix) 
    print("topic_name_prefix = %s" % topic_name_prefix) 

    pub_stop = rospy.Publisher(topic_name_prefix +'stop', RobotStop, queue_size=10)           
    
    #print 'wait services'
    rospy.wait_for_service(srv_name_prefix +'/motion/move_joint')

    move_joint  = rospy.ServiceProxy(srv_name_prefix +'/motion/move_joint', MoveJoint)
    move_jointx = rospy.ServiceProxy(srv_name_prefix +'/motion/move_jointx', MoveJointx)

    move_line   = rospy.ServiceProxy(srv_name_prefix +'/motion/move_line', MoveLine)
    move_circle = rospy.ServiceProxy(srv_name_prefix +'/motion/move_circle', MoveCircle)
    move_spline_joint = rospy.ServiceProxy(srv_name_prefix +'/motion/move_spline_joint', MoveSplineJoint)   
    move_spline_task = rospy.ServiceProxy(srv_name_prefix +'/motion/move_spline_task', MoveSplineTask)

    move_blending = rospy.ServiceProxy(srv_name_prefix +'/motion/move_blending', MoveBlending)
    move_spiral = rospy.ServiceProxy(srv_name_prefix +'/motion/move_spiral', MoveSpiral)
    move_periodic = rospy.ServiceProxy(srv_name_prefix +'/motion/move_periodic', MovePeriodic)
