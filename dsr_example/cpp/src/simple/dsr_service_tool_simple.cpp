...
{   
    ...
    robot.config_create_tool("tool1", 5.3, fCog, fInertia);
    robot.config_create_tcp("tcp1", p0);
    robot.set_current_tool("tool1");
    robot.set_current_tcp("tcp1");

    ROS_INFO("current tool : %s", robot.get_current_tool().c_str());
    ROS_INFO("current tcp : %s", robot.get_current_tcp().c_str());
    while(ros::ok())
    {            
    }
