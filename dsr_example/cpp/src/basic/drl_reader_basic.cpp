...
std::string fileRead(){
    ifstream ifs;
    ifs.open("drl_files/movej_test.drl");
    std::string drl_code = "";
    std::string temp;
    ...
    ifs.close();
    std::cout << drl_code << endl;
    return drl_code;
} 


int set_robot_mode(int robot_mode)
{
    ros::NodeHandlePtr node = boost::make_shared<ros::NodeHandle>();
    ros::ServiceClient srvSetRobotMode = node->serviceClient<dsr_msgs::SetRobotMode>("/"+ROBOT_ID +ROBOT_MODEL+"/system/set_robot_mode");
...
{
    ros::NodeHandlePtr node = boost::make_shared<ros::NodeHandle>();
    ros::ServiceClient srvGetRobotMode = node->serviceClient<dsr_msgs::GetRobotMode>("/"+ROBOT_ID +ROBOT_MODEL+"/system/get_robot_mode");
...
{   
    ...
    //----- init ROS ---------------------- 
    ros::init(argc, argv, "drl_reader_basic_cpp", ros::init_options::NoSigintHandler);
    ros::NodeHandle nh("~");
    ...
    drl_start(ROBOT_SYSTEM_REAL, fileRead());
    ...
    thread_sub.join();
    ROS_INFO("drl_reader_simple finished !!!!!!!!!!!!!!!!!!!!!");
    return 0;
}
