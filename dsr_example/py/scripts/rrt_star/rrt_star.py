...
class RRTStar:
    ...
    def steer(self, from_node, to_node, extend_length=float("inf")):
        ...
        if extend_length > d:
            extend_length = d
        #n_expand indica quanti passi servono per coprire la distanza extend_length.
        #self.path_resolution definisce la granularità del movimento (quanto si avanza per step).
        #max(2, ...) assicura che ci siano almeno 2 punti nel percorso.
    ...
    def check_collision(node, robot, obstacleList, interpolation_steps=5, sphere_radius=0.15):
        ...
        for config in node.path_x:
            ...
            x_list, y_list, z_list = robot.get_points(config)
